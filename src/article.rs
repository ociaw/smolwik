...
impl RawArticle {
    pub async fn read_from_path(filepath: &Path, path: &str) -> Result<RawArticle, ArticleReadError> {
        let mut file = filesystem::ReadableFile::open(filepath)
            .await
            .map_err(|source| match source.kind() {
                io::ErrorKind::NotFound => ArticleReadError::NotFound { path: path.to_owned() },
                _ => ArticleReadError::IoError {
                    source,
                    path: path.to_owned(),
                },
            })?;
        ...
        R: io::AsyncBufRead + Unpin,
    {
        ...
        if !str.eq(MARKDOWN_SEPARATOR_LINUX) && !str.eq(MARKDOWN_SEPARATOR_WINDOWS) {
            eprintln!(
                "Metadata start not found. Expected\n{}, found\n{}",
        ...
        let metadata = toml::from_str(&metadata).with_context(|_| InvalidMetadataSnafu { path: path.to_owned() })?;
...
pub enum ArticleReadError {
    ...
    #[snafu(display(
        "The start of the article metadata was not found in {}. Expected \n+++\nBut got\n{}",
        path,
        first_line
    ))]
...
pub enum ArticleWriteError {
    ...
    ConflictingWriteInProgress { path: String },
    /// Indicates that the article's path is invalid.
    #[snafu(display("Invalid file path {}: {}", path, source))]
    InvalidPath { source: FileWriteError, path: String },
    /// Indicates that there was an error reading the file.
    #[snafu(display("Error when writing to {}: {}", path, source))]
    UnhandlableWriteError { source: FileWriteError, path: String },
}
    ...
    pub fn from_file_write_error(err: FileWriteError, path: String) -> ArticleWriteError {
        ...
        match &err {
            ...
            FileWriteError::UnhandlableWriteError { source, filepath: _ } => match source.kind() {
                ErrorKind::NotFound | ErrorKind::IsADirectory | ErrorKind::InvalidInput | ErrorKind::InvalidFilename => {
                    InvalidPath { source: err, path }
                }
